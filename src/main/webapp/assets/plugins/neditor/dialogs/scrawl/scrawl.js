...
function exec(scrawlObj) {
    if (scrawlObj.isScrawl) {
        ...
        if(browser.ie){
        	...
        	file = scrawlObj.dataURLtoFile(base64, 'scrawl-image.png');
        }
       ...
       if(browser.ie){
    	   /* 上传涂鸦图片 FOR IE */
    	   var fileName="scrawl-image.png";
           editor.getOpt("scrawlUploadService")(scrawlObj, editor).uploadScrawForIE(fileName,file,successCall, errorCall);
       }else{
    	   /* 上传涂鸦图片 */
           editor.getOpt("scrawlUploadService")(scrawlObj, editor).uploadScraw(file, base64, successCall, errorCall);
       }
    } else {
